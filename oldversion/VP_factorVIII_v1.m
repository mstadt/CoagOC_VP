...
MEAN_lev_range = [max(MEAN_lev - 0.5, 0.95*MEAN_lev),...
                        min(MEAN_lev +  1.0, 1.2*MEAN_lev)]; 
                    % NOTE: I set top of MEAN_lev range higher because STD
                    % is high and tends to lower the mean
...
fac_range = {[50, 195], [50, 195], [53, 200]}; % range allowed for factor values
bw = [0.65, 0.8, 0.8]; %[0.5, 0.65, 0.85]; %[0.5, 0.65, 0.8]; %[0.6, 0.7, 0.8]; %[0.6, 0.7, 0.7]; %[0.5, 0.5, 0.5]; %[0.40, 0.40, 0.40]; % bandwidth values for noOC, lev, dsg
...
for ii = 1:length(F_noOC)
    ...
    if noOC > 150
        if noOC > 180
            ...
            weight_noOC(ii) = 0.5; % decrease weight when greater than 150
        end
    end
...
for ii = 1:length(F_dsg)
    ...
    if dsg > 150
        if dsg > 180 
            weight_dsg(ii) = 0.5; % decrease weight of greater than 180
        elseif dsg < 170
            weight_dsg(ii) = 0.8;
        end
...
fprintf('mean diff lev: %0.3f, MEAN_lev: %0.1f\n', mean_diff_lev, MEAN_lev)
fprintf('mean diff dsg: %0.3f, MEAN_dsg: %0.1f\n', mean_diff_dsg, MEAN_dsg)
...
% Hyperparameters
sigma_lev = 0.26; %0.27; %0.25;
sigma_dsg = 0.231; %0.21;
...
fprintf('final mean diff lev: %0.2f, MEAN_lev: %0.2f \n', mean(diff_lev), MEAN_lev)
fprintf('final mean diff dsg: %0.2f, MEAN_dsg: %0.2f \n', mean(diff_dsg), MEAN_dsg)
