...
MEAN_lev_range = [min(MEAN_lev +  0.5, 1.1*MEAN_lev),...
                    max(MEAN_lev - 0.5, 0.9*MEAN_lev)]; 
...
MEAN_dsg = -11;
MEAN_dsg_range = [min(1.05*MEAN_dsg, MEAN_dsg + 0.5),...
                        max(0.95 * MEAN_dsg, MEAN_dsg - 0.5)];
STD_dsg  = 8;
...
fac_range = {[50, 175], [50, 160], [50, 150]}; % range allowed for factor values
bw = [0.50, 0.60, 0.50]; %[0.40, 0.40, 0.40]; % bandwidth values for noOC, lev, dsg
